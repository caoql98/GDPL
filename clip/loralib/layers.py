...
class MultiheadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None, vdim=None, lora_alpha: int = 1, r=0):
        ...
        if self._qkv_same_embed_dim is False:
            ...
            self.q_proj_weight_lora_S = nn.Parameter(torch.ones(r))
            self.k_proj_weight_lora_S = nn.Parameter(torch.ones(r))
            self.v_proj_weight_lora_S = nn.parameter(torch.ones(r))
            self.register_parameter('in_proj_weight', None)
            self.register_parameter('in_proj_weight_lora_A', None)
            self.register_parameter('in_proj_weight_lora_B', None)
            self.register_parameter('in_proj_weight_lora_S', None)
        else:
            self.in_proj_weight = nn.Parameter(torch.empty(3 * embed_dim, embed_dim))
            self.in_proj_weight_lora_A = nn.Parameter(torch.empty(r, embed_dim))
            self.in_proj_weight_lora_S = nn.Parameter(torch.ones(r))
            ...
            self.register_parameter('q_proj_weight_lora_S', None)
            self.register_parameter('k_proj_weight_lora_S', None)
            self.register_parameter('v_proj_weight_lora_S', None)
        if bias:
    ...
    def _reset_parameters(self):
        if self._qkv_same_embed_dim:
            ...
            # xavier_uniform_(self.in_proj_weight_lora_S)
        else:
            ...
            # xavier_uniform_(self.q_proj_weight_lora_S)
            # xavier_uniform_(self.k_proj_weight_lora_S)
            # xavier_uniform_(self.v_proj_weight_lora_S)
    ...
    Shape:
        ...
        if not self._qkv_same_embed_dim:
            return multi_head_attention_forward(
                ...
                q_proj_weight_S=self.q_proj_weight_lora_S, k_proj_weight_S=self.k_proj_weight_lora_S, v_proj_weight_S=self.v_proj_weight_lora_S,
                q_proj_weight_scaling=self.scaling, k_proj_weight_scaling=self.scaling, v_proj_weight_scaling=self.scaling
            )
        else:
            return multi_head_attention_forward(
                query, key, value, self.embed_dim, self.num_heads,
                self.in_proj_weight, self.in_proj_bias, self.in_proj_weight_lora_A, self.in_proj_weight_lora_B, self.in_proj_weight_lora_S, self.scaling,
                self.bias_k, self.bias_v, self.add_zero_attn,
                self.dropout, self.out_proj.weight, self.out_proj.bias, self.out_proj.lora_A, self.out_proj.lora_B, self.out_proj.scaling,
                training=self.training,
                key_padding_mask=key_padding_mask, need_weights=need_weights,
...
def multi_head_attention_forward(query: Tensor,
                                 ...
                                 q_proj_weight_S: Optional[Tensor] = None,
                                 k_proj_weight_S: Optional[Tensor] = None,
                                 v_proj_weight_S: Optional[Tensor] = None,
    ...
    if not use_separate_proj_weight:
        if torch.equal(query, key) and torch.equal(key, value):
            ...
            qkv += linear(linear(linear(query, in_proj_weight_A), torch.diag_embed(in_proj_weight_S)), in_proj_weight_B) * in_proj_weight_scaling / 4.0
            ...
            q += linear(linear(linear(query, in_proj_weight_A), torch.diag_embed(in_proj_weight_S)), in_proj_weight_B[_start:_end, :]) * in_proj_weight_scaling / 4.0
            ...
            if key is None:
                ...
                if _b is not None:
                    _b = _b[_start:]
                kv = linear(key, _w, _b)
                kv += linear(linear(linear(key, in_proj_weight_A), torch.diag_embed(in_proj_weight_S)), in_proj_weight_B[_start:, :]) * in_proj_weight_scaling / 4.0
            ...
            if _b is not None:
                _b = _b[_start:_end]
            k = linear(key, _w, _b)
            k += linear(linear(linear(key, in_proj_weight_A), torch.diag_embed(in_proj_weight_S)), in_proj_weight_B[_start:_end, :]) * in_proj_weight_scaling / 4.0
            ...
            v += linear(linear(linear(value, in_proj_weight_A), torch.diag_embed(in_proj_weight_S)), in_proj_weight_B[_start:, :]) * in_proj_weight_scaling / 4.0
    else:
        ...
        q_proj_weight_non_opt_S = torch.jit._unwrap_optional(q_proj_weight_S)
        k_proj_weight_non_opt_S = torch.jit._unwrap_optional(k_proj_weight_S)
        v_proj_weight_non_opt_S = torch.jit._unwrap_optional(v_proj_weight_S)
        ...
        q += linear(linear(linear(query, q_proj_weight_non_opt_A), torch.diag_embed(in_proj_weight_S)), q_proj_weight_non_opt_B) * q_proj_weight_scaling / 4.0
        k += linear(linear(linear(key, k_proj_weight_non_opt_A), torch.diag_embed(in_proj_weight_S)), k_proj_weight_non_opt_B) * k_proj_weight_scaling / 4.0
        v += linear(linear(linear(value, v_proj_weight_non_opt_A), torch.diag_embed(in_proj_weight_S)), v_proj_weight_non_opt_B) * v_proj_weight_scaling / 4.0
