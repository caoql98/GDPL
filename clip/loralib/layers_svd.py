...
class MultiheadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None, vdim=None, lora_alpha: int = 1, r=0):
        ...
        if self._qkv_same_embed_dim is False:
            ...
            self.q_proj_weight_lora_S = nn.Parameter(torch.Tensor(r, r))
            self.k_proj_weight_lora_S = nn.Parameter(torch.Tensor(r, r))
            self.v_proj_weight_lora_S = nn.Parameter(torch.Tensor(r, r))
            ...
            self.in_proj_weight_lora_S = nn.Parameter(torch.empty(r, r))
...
def multi_head_attention_forward(query: Tensor,
    ...
    if not use_separate_proj_weight:
        if torch.equal(query, key) and torch.equal(key, value):
            ...
            qkv += linear(linear(linear(query, in_proj_weight_A), in_proj_weight_S), in_proj_weight_B) * in_proj_weight_scaling
            ...
            q += linear(linear(linear(query, in_proj_weight_A), in_proj_weight_S), in_proj_weight_B[_start:_end, :]) * in_proj_weight_scaling
            ...
            if key is None:
                ...
                if _b is not None:
                    _b = _b[_start:]
                kv = linear(key, _w, _b)
                kv += linear(linear(linear(key, in_proj_weight_A), in_proj_weight_S), in_proj_weight_B[_start:, :]) * in_proj_weight_scaling
            ...
            if _b is not None:
                _b = _b[_start:_end]
            k = linear(key, _w, _b)
            k += linear(linear(linear(key, in_proj_weight_A), in_proj_weight_S), in_proj_weight_B[_start:_end, :]) * in_proj_weight_scaling
            ...
            v += linear(linear(linear(value, in_proj_weight_A), in_proj_weight_S), in_proj_weight_B[_start:, :]) * in_proj_weight_scaling
    else:
        ...
        q += linear(linear(linear(query, q_proj_weight_non_opt_A), q_proj_weight_non_opt_S), q_proj_weight_non_opt_B) * q_proj_weight_scaling
        k += linear(linear(linear(key, k_proj_weight_non_opt_A), k_proj_weight_non_opt_S), k_proj_weight_non_opt_B) * k_proj_weight_scaling
        v += linear(linear(linear(value, v_proj_weight_non_opt_A), v_proj_weight_non_opt_S), v_proj_weight_non_opt_B) * v_proj_weight_scaling
